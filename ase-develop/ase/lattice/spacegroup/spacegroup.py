    ...
    def equivalent_sites(self, scaled_positions, ondublicates='error', 
        ...
        for kind, pos in enumerate(scaled):
            for rot, trans in self.get_symop():
                ...
                #for i in site:
                #    if site[i]>0.999999999:
                #        site[i]=0.0
                #if np.any(site>1.0-symprec2):
                #    print "XXX",kind,site,np.dot(rot, pos) + trans
                #print kind,site,np.dot(rot, pos) + trans
                if not sites:
                    sites.append(site)
                    kinds.append(kind)
                    continue
        ...
        for j, s in enumerate (sym.split(sep)):
            ...
            while s:
                ...
                if s[0] in 'xyz':
                    ...
                    #elif (t.startswith('0.33')) or (t.startswith('.33')):
                    #    trans[i,j] =1/3
                    #elif (t.startswith('0.66')) or (t.startswith('.66')):
                    #    trans[i,j] =2/3
                    else:
                        trans[i,j] = float(t)
                    trans[i,j] = trans[i,j]*sign
                else:
                    raise SpacegroupValueError(
