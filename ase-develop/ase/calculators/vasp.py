...
float_keys = [
    ...
    'pstress',    # IO: set pressure in kbar
...
bool_keys = [
    ...
    'lnoncollinear', #IO switch for noncollinear calculations
    ...
    def write_incar(self, atoms, **kwargs):
        ...
        for key, val in self.list_params.items():
            if val is not None:
                ...
                elif key in ('ldauu', 'ldauj', 'ldaul'): #IO and \
                    #IO not self.dict_keys.has('ldau_luj'):
                    [incar.write('%.4f ' % x) for x in val]
